...
const refreshToken = async () => {
  try {
    ...
    console.error("Token refresh failed", error);
    window.location.href = "/login"; // Redirect to login on refresh failure
    return null; // Ensure you return a value for consistency
  }
};
...
export const registerUser = (data) => api.post("register/", data);
export const createProfile = (data) => api.post("profile/create/", data);
export const updateProfile = (data) => api.put("profile/update/", data); // Removed id parameter, adjust if needed
